        ...
        <ul class="nav-ul" id="navUL" name="navUL">
			...
			<li>
				...
				<select name="admin" id="admin" style="display:none; font-size:1rem; font-family: 'PT Sans', sans-serif" onchange="goToCMS()">
					...
					<option value="-1" <?php if($val == '-1') echo 'selected'; ?>>-Admin Tools-</option>
					<option value="add" <?php if($val == 'add') echo 'selected'; ?>>Add Apt, Bldg, and/or Neighborhood</option>
					<option value="apt" <?php if($val == 'apt') echo 'selected'; ?>>Edit Apartment Details</option>
					<option value="bldg" <?php if($val == 'bldg') echo 'selected'; ?>>Edit Building Details</option>
				</select>
			</li>
            <script>
             ...
             let val = event.target.value;
                if(val == 'add') {
                   document.querySelector(`[value^='add']`).selected = true; 
                } else if(val == 'apt') {
                   document.querySelector(`[value^='apt']`).selected = true; 
                } else if(val == 'bldg') {
                   document.querySelector(`[value^='bldg']`).selected = true; 
                }
